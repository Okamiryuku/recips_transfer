...
def transformar_recetas():
    ...
    receta_actual = None
    codigo_actual = None
    primera_fila_receta = True  # Para controlar si es la primera fila de la receta
    ...
    for idx, row in df_recetas.iterrows():
        ...
        if pd.notna(valores[0]) and 'Nº de receta:' in str(valores[0]):
            ...
            primera_fila_receta = True  # Resetear para la nueva receta
        ...
        # Verificar si es una fila de ingrediente (tiene datos en las columnas relevantes)
            ...
            if (ingrediente and
                ...
                # Determinar si poner el nombre de la receta o dejarlo vacío
                nombre_receta_para_fila = receta_actual if primera_fila_receta else ''

                # Crear la fila transformada
                fila = {
                    'Familia': 'Recetas',  # Valor por defecto
                    'Clave Softrestaurant': '',  # Vacío como en el ejemplo
                    'Producto/Subreceta': nombre_receta_para_fila,
                ...
                primera_fila_receta = False  # Después de la primera fila, ya no mostrar el nombre
    ...
    print(df_nuevas_recetas[['Producto/Subreceta', 'Insumo O subreceta', 'Cant', 'UmED']].head(10))
